...
public sealed class GrafikUwagaMiesiecznaRepository(BoberConnectionFactory connectionFactory)
    : IGrafikUwagaMiesiecznaRepository
{
    public async Task<IReadOnlyList<GrafikUwagaMiesieczna>> GetByZmianaAndMonthAsync(
        int zmianaId,
        int rok,
        int miesiac,
        CancellationToken cancellationToken = default)
    {
        ...
        var result = new List<GrafikUwagaMiesieczna>();
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            result.Add(new GrafikUwagaMiesieczna
            {
    ...
    public async Task UpsertAsync(GrafikUwagaMiesieczna uwaga, CancellationToken cancellationToken = default)
    {
        await using var connection = connectionFactory.CreateOpenConnection();

        await using var checkCmd = new OleDbCommand(
            """
            SELECT COUNT(*) FROM GrafikUwagiMiesieczne
        ...
        if (exists)
        {
            await using var updateCmd = new OleDbCommand(
                """
                UPDATE GrafikUwagiMiesieczne SET Tresc = ?
                WHERE FunkcjonariuszId = ? AND ZmianaId = ? AND Rok = ? AND Miesiac = ?
                """,
                connection);
        ...
        else
        {
            await using var insertCmd = new OleDbCommand(
                """
                INSERT INTO GrafikUwagiMiesieczne (FunkcjonariuszId, ZmianaId, Rok, Miesiac, Tresc)
                VALUES (?, ?, ?, ?, ?)
                """,
                connection);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var command = new OleDbCommand(
            """
            DELETE FROM GrafikUwagiMiesieczne
            WHERE FunkcjonariuszId = ? AND ZmianaId = ? AND Rok = ? AND Miesiac = ?
            """,
            connection);
